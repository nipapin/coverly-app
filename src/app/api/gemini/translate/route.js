...
export async function POST(req) {
  try {
    const { texts, languages, image } = await req.json();

    if (!languages || languages.length === 0) {
      return NextResponse.json({ error: "Please use at least 1 language to translate" }, { status: 500, headers: corsHeaders });
    }

    // 1. Input Validation
    if (Array.isArray(texts) && texts.length !== 0) {
      return handleText({ texts, languages });
    }

    if (image) {
      return handleImage({ image, languages });
    }

    return NextResponse.json({ error: "Please use texts or image as source to translate" }, { status: 500, headers: corsHeaders });
  } catch (error) {
...
const handleText = async ({ texts, languages }) => {
...
const handleImage = async ({ image, languages }) => {
  const prompt = `Update this image to be in ${languages[0]}. Do not change any other elements of the image.`;
};
