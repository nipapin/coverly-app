...
export const useImageTransform = ({ layer }) => {
	...
	const fitVertical = () => {
		...
		layerSource.setAttrs({ scaleX: ratio, scaleY: ratio, x: layerGroup.width() / 2, y: 0, offsetX: layerSource.width() / 2, offsetY: 0 });
		handleTransformEnd({ target: layerSource });
	};
	const fitHorizontal = () => {
		resetTransform();
		...
		layerSource.setAttrs({ scaleX: ratio, scaleY: ratio, x: 0, y: layerGroup.height() / 2, offsetY: layerSource.height() / 2, offsetX: 0 });
		handleTransformEnd({ target: layerSource });
	};
	return {
