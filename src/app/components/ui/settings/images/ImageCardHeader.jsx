...
export default function ImageCardHeader({ src, name, count }) {
		...
		const modifiedTemplate = {
			...
			layers: template.layers.map((_layer) => {
				if (_layer.name === name) {
					...
					return { ..._layer, children: _layer.children.filter((child) => child.src !== src) };
				}
				return _layer;
			})
		};
	...
	const handleUpload = async (e) => {
		...
		const modifiedTemplate = {
			...
			layers: template.layers.map((_layer) => {
				if (_layer.name === name) {
					return { ..._layer, children: _layer.children.map((child) => ({ ...child, src: data.url, variants: [{ src: data.url }] })) };
				}
				return _layer;
			})
		};
