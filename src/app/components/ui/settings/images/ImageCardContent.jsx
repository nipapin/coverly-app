    ...
    const modifiedTemplate = {
      ...
      layers: template.layers.map((templateLayer) => {
        ...
        const children = templateLayer.children.map((child) =>
          child.type === "image"
            ? { ...child, variants: child.variants.filter((variant) => variant.src !== variantSrc) }
            : child
        );
    ...
    <CardContent sx={{ display: "flex", flexDirection: "column", gap: "0.5rem", pt: 0 }}>
      ...
      {variants.length > 0 ? (
        <Box>
          ...
          <Box
            sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, 60px)", gap: "0.5rem" }}
          >
            {variants.map((variant, index) => (
              ...
              >
                <Card
                  sx={{ border: `1px solid ${variant.src === src ? "white" : "transparent"}` }}
                  variant="outlined"
                >
