					...
					<Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, 60px)", gap: "0.5rem" }}>
						{variants.map((variant, index) =>
							index > 0 ? (
								...
								>
									<VariantCard variant={variant} src={src} onClick={() => handleRemove(variant.src)} />
								</Badge>
							) : (
								<VariantCard key={variant.src} variant={variant} src={src} name={name} onClick={() => handleSelect(variant.src)} />
							)
						)}
					</Box>
				</Box>
